...
object Backbone{
    ...
    fun getBackboneKaiKue(cs: ClauseSetWatchedLiterals, useOptimizations:Boolean = true):Set<Literal> {
        ...
        map {Literal(it.affectedVariable, it.value) })
        {
            ...
            if (changedFormula.isEmpty) {
                ...
                filter { retu.map {lit -> lit.first }.contains(it.affectedVariable) || skippedVars.contains(it.affectedVariable) }.
                filter { defaultSettings.contains(Literal(it.affectedVariable,it.value)) }.
                forEach{toSkip ->
    ...
    private fun makeAdjustedClauseSet(cs: ClauseSetWatchedLiterals, toNegate: Literal): ClauseSetWatchedLiterals {
        val negatedLiteral = Literal(toNegate.variable,!toNegate.predicate)
/*
    //var clauses: MutableList<materials.Clause> = cs.copyClauses()
    ...
    if (isWatchedLiterals) {
        return materials.ClauseSetWatchedLiterals(clauses.map { it -> it as materials.ClauseWatchedLiterals }.toTypedArray())
    } else
        return materials.ClauseSet(clauses.toTypedArray())*/
    ...
    fun getBackboneIntersections(cs: ClauseSetWatchedLiterals):Set<Literal> {
        ...
        val firstPrimeImplicant:Set<Variable> = Implicant.getPrimeImplicantWithWatchedLiterals(cs,currentTable).
        map { it.variable }.toSet()

        //have a set of literals that need to be rebutted, meaning that we try to
        // create a prime implicant, that has that materials.getVariable with the opposite setting.
        ...
        while (rebuttals.isNotEmpty()) {
            //2. run the CDCL on the SAME formula (with learned clauses, but reset materials.getVariable settings)
